...
![process-vm-mapping](/assets/linux-source-code/process_vm.jpg)
...
![vm_address](/assets/linux-source-code/vm_address.png)
...
![vma-pma-maping](/assets/linux-source-code/vma-pma-maping.png)
...
![read-write-system-call](/assets/linux-source-code/read-write-system-call.png)

而调用 `mmap()` 系统调用对文件进行映射后，用户对映射后的内存进行读写实际上是对文件缓存的读写，所以减少了一次系统调用，从而加速了对文件读写的效率。如下图：

![mmap-memory-address](/assets/linux-source-code/mmap-memory-address.png)
