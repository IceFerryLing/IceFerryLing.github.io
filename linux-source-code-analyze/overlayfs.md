...
![overlayfs-map](/assets/images/linux-source-code/overlayfs-map.png)
...
前面介绍过挂载 `OverlayFS` 文件系统的命令，挂载 `OverlayFS` 文件系统会触发系统调用 `sys_mount()`，而 `sys_mount()` 会执行 `虚拟文件系统` 的通用挂载过程，如申请和初始化 `超级块对象(super block)`（可参考：[虚拟文件系统](/series/linux-source-code-analyze/virtual_file_system/)）。然后调用具体文件系统的 `fill_super()` 接口来填充 `超级块对象`，对于 `OverlayFS` 文件系统而言，最终会调用 `ovl_fill_super()` 函数来填充 `超级块对象`。
...
![overlayfs-relation](/assets/images/linux-source-code/overlayfs-relation.jpg)
...
![overlayfs-mount](/assets/images/linux-source-code/overlayfs-mount.jpg)
