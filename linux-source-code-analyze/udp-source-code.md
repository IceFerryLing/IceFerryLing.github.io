...
![tcp-ip-layer](/assets/images/linux-source-code/udp/tcp-ip-layer.png)

也就是说 `UDP协议` 是建立中 `IP协议`（网络层）之上的，`IP协议` 用于区分网络上不同的主机（[IP协议源码分析](/series/linux-source-code-analyze/ip-source-code/)），而 `UDP协议` 用于区分同一台主机上不同的进程发送（接收）的网络数据，如下图所示：

![udp-schedule](/assets/images/linux-source-code/udp/udp-schedule.png)
...
![udp-header](/assets/images/linux-source-code/udp/udp-header.png)
...
![udp-header](/assets/images/linux-source-code/udp/udp-header-2.png)
...
![udp-sendmsg](/assets/images/linux-source-code/udp/udp-sendmsg.png)
...
`ip_build_xmit()` 函数是 IP 协议层的实现，这里就不作说明，可以参考此文章：[IP协议源码分析](/series/linux-source-code-analyze/ip-source-code/)。
...
![udp-recv-process](/assets/images/linux-source-code/udp/udp-recv-process.png)
