...
![buffered-io](/assets/images/linux-source-code/buffered-io.png)
...
![buffered-io-2](/assets/images/linux-source-code/buffered-io-2.jpg)
...
打开文件的整个流程比较复杂，但对我们分析 `直接I/O` 并没有太大关系，之前在虚拟文件系统一章已经分析过，这里就不再重复了，可以参考之前的文章：[虚拟文件系统](/series/linux-source-code-analyze/virtual_file_system/)
...
`map_user_kiobuf()` 函数属于内存管理部分，可以参考之前的 [内存管理](/series/linux-source-code-analyze/virtual_memory_address_manager/) 相关的文章进行分析，这里就不重复了。
