...
![epoll-eventpoll](/assets/images/linux-source-code/epoll-eventpoll.jpg)
...
```
每个 `socket` 对象都有个等待队列（`waitqueue`, 关于等待队列可以参考文章: [等待队列原理与实现](/series/linux-source-code-analyze/waitqueue/)），用于存放等待 socket 状态更改的进程。
...
![epoll-principle](/assets/images/linux-source-code/epoll_principle.jpg)
