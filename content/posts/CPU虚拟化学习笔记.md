...
description: 记录学习 CPU 虚拟化时，对 vCPU、VM Exit、调度、I/O 路径和排障方法的理解。
category:
  - 操作系统
tags:
   - CPU虚拟化
...
学 CPU 虚拟化的时候，我一开始最容易把它理解成“把一个物理 CPU 切成很多份”。这个说法虽然能帮我入门，但很快就不够用了：虚拟机为什么会发生 VM Exit？vCPU 和 pCPU 到底怎么调度？为什么有时 CPU 利用率不高，业务延迟却很高？

这篇笔记按我的学习过程来整理：先建立 vCPU、pCPU、Hypervisor 的关系，再看调度算法、VM Exit、I/O 虚拟化、NUMA 和排障指标。重点不是背概念，而是把“机制为什么存在”和“性能问题怎么定位”连起来。

**定位**：学习笔记 / 操作系统复盘 / 排障框架  
**阅读建议**：先通读一次建立地图，再按章节回看图解与案例  
...
学完这部分以后，我感觉 CPU 虚拟化真正难的不是定义，而是把“执行权、特权切换、调度权”放在一条链路上看。Guest 里看到的是 vCPU，但真正决定它什么时候运行的是 Host 和 Hypervisor；Guest 以为自己在执行普通指令，但某些敏感操作会触发 VM Exit；业务以为自己只是 CPU 慢，但背后可能是双层调度、I/O 中断或 NUMA 迁移。

如果能做到下面 4 件事，说明这篇已经从“看懂”走向“会用”：
