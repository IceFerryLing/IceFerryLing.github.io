...
description: 记录学习现代大语言模型架构与输入输出流水线时的理解、困惑和阶段性总结。
category:
  - 人工智能
tags:
   - LLM
...
刚开始学大语言模型的时候，我最容易被各种名词淹没：Tokenizer、RoPE、RMSNorm、SwiGLU、KV Cache、RLHF、DPO、FlashAttention、PagedAttention。每个概念单独看都能找到解释，但真正困难的是把它们串成一条完整链路：一段文本进来以后，到底经历了哪些步骤，为什么现代模型会选择这些模块，它们分别解决什么问题。

所以这篇笔记不是单纯罗列大模型组件，而是按我的学习路径来整理：先从 Transformer 和自回归建模建立主线，再逐步补上分词、位置编码、注意力、FFN、归一化、训练、后训练、推理和服务化优化。写这篇时我一直在追问三个问题：

1. 这个模块解决的原始问题是什么？
2. 它在数学或工程上具体怎么工作？
3. 如果没有它，模型训练或推理会在哪里卡住？

现代大语言模型（Large Language Model, LLM）通常以 Transformer 为核心架构，通过大规模自监督预训练学习自然语言、代码、数学表达式和结构化文本中的统计规律，再经过指令微调、偏好对齐和系统级推理优化，形成可交互的文本生成系统。下面的内容重点不在经验性配置罗列，而在记录我如何把这些模块理解成一条从输入到输出的流水线。
...
这次梳理之后，我对大模型的理解从“很多高级模块堆在一起”变成了“一条不断变换表示的流水线”。

输入侧最关键的是把自然语言变成 token 和向量；模型主体负责在多层 Transformer block 中不断更新表示；训练阶段通过下一个 token 预测学习统计规律；后训练再把“会续写文本”的模型调成“更能按指令回答”的助手；推理服务阶段则重点处理 KV Cache、显存占用和吞吐。

我觉得最容易混淆的地方有三个：

- 预训练目标和助手行为不是一回事，后训练是在修正目标错配；
- 长上下文、KV Cache、PagedAttention 更多是系统效率问题，不等于模型一定更会推理；
- Attention、FFN、Norm、Residual 不是孤立模块，它们共同决定表示如何更新、梯度如何传播、训练是否稳定。

后面再看新的模型结构时，我会先问它到底改了流水线中的哪一段：是表示方式、注意力计算、训练目标、推理效率，还是服务化内存管理。这样比单独记名词更不容易散。
