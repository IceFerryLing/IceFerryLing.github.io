...
description: 记录学习 Linux 内核 struct page 时，对字段含义、页生命周期和内存管理主线的理解。
category:
  - 操作系统
tags:
   - 内存管理
   - 操作系统
---
# Linux 内核 struct page 学习笔记

学 Linux 内存管理时，`struct page` 是我一开始最容易被字段数量劝退的结构体。它看起来只是一个结构体，但实际上牵着伙伴系统、页缓存、匿名页、LRU 回收、缺页处理和 IO 等很多路径。

这篇笔记记录我理解 `struct page` 的过程：先把它看成“物理页在内核里的身份证”，再按字段去看它如何记录页的归属、状态、引用关系和生命周期。

`struct page` 是 Linux 内核**内存管理子系统的核心结构**。系统中**每一个**物理内存页（Page Frame，通常大小为 4KB），在内存中都对应一个 `struct page` 结构体实例。
...
- `struct page` 不是“保存页内容”的结构，而是“描述页状态”的结构。
- `mapping`、`index` 让我理解了页和地址空间之间的关系。
- `count`、`_mapcount` 一类字段决定了页什么时候还能被引用、什么时候可能被回收。
- `flags` 把很多页状态压缩到位标志里，是理解页锁定、脏页、引用、回收路径的关键。
- `list`、`lru` 说明同一个页会在不同生命周期中挂到不同链表上。

学完以后，我对 `struct page` 的感觉是：它不是孤立的数据结构，而是 Linux 内存管理各条路径交汇的入口。理解它以后，再看伙伴系统、页缓存、匿名页、LRU 回收和缺页处理，会更容易把这些机制连起来。

后面继续看内核源码时，我会优先追踪三个问题：这个页属于谁，当前处于什么状态，还有谁在引用它。多数 `struct page` 字段最后都能回到这三个问题上。
